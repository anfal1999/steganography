...
{
    printf("INFO    : Decode magic string\n");
...
{
   // printf("INFO    : Decode data from image\n");
    int i;
    char str[8];
    for (i = 0; i < size; i++)
    {
...
{
    printf("INFO    : Decode file extension size\n");
    char str[32];
    int length;
...
{
    printf("INFO    : Decode secrete file extension\n");
    file_ext = ".txt";
...
{
    printf("INFO    : Decode secrete file size\n");
    char str[32];
...
{
    printf("INFO    : Decode secrete file data\n");
    char ch;
...
Status_d do_decoding(DecodeInfo *decInfo)
{
    if (open_files_dec(decInfo) == d_success)
    {
        ...
        {
            printf("Success : Decoding magic string Successfull\n");
            if (decode_file_extn_size(strlen(".txt"), decInfo->fptr_d_src_image) == d_success)
            {
                printf("Success : Decoding file extension size Successfull\n");
                if (decode_secret_file_extn(decInfo->d_extn_secret_file, decInfo) == d_success)
                {
                    printf("Success : Decoding Secret File Extension Successfull\n");
                    if (decode_secret_file_size(decInfo->size_secret_file, decInfo) == d_success)
                    {
                        printf("Success : Decoding secret file size Successfull\n");
                        if (decode_secret_file_data(decInfo) == d_success)
                        {
                            printf("Success : Decoding secret file data Successfull\n");
                        }
                        else
                        {
